...
export function jsonInterval<A>(dictA: Json<A>): Json<Interval<A>> {
  return {
    ...
    fromJson: (value) => {
      const from = Prelude.caseFieldWithValue(
        "from",
        jsonLowerBound(dictA).fromJson,
        value,
      );
      const to = Prelude.caseFieldWithValue(
        "to",
  ...
  return {
    toJson: (extended) => {
      switch (extended.name) {
        case "NegInf":
          return Prelude.jsonConstructor(extended.name, []);
        case "PosInf":
          return Prelude.jsonConstructor(extended.name, []);
        case "Finite":
          return Prelude.jsonConstructor(extended.name, [
    ...
    fromJson: (value) => {
      return Prelude.caseJsonConstructor<Extended<A>>("Plutus.V1.Extended", {
        "NegInf": (ctorfields) => {
...
export function eqLowerBound<A>(dictA: Eq<A>): Eq<LowerBound<A>> {
  return Prelude.eqPair(eqExtended(dictA), Prelude.eqBool);
}
  ...
  return {
    toJson: (lowerBound) => {
      return {
        ...
        closed: Prelude.jsonBool.toJson(lowerBound[1]),
      };
    },
    fromJson: (value) => {
      const bound = Prelude.caseFieldWithValue(
        "bound",
        jsonExtended(dictA).fromJson,
        value,
      );
      const closed = Prelude.caseFieldWithValue(
        "closed",
        Prelude.jsonBool.fromJson,
        value,
      );
      return [bound, closed];
    },
...
export function isPlutusDataLowerBound<A>(
  ...
  return PreludeInstances.isPlutusDataPairWithTag(
